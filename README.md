...
## 第六轮：Challenge Set 与 Mutation Testing

第六轮增加了挑战集生成能力。`mutate` 命令会基于现有 Benchmark 自动生成更难的样本，并输出一份 mutation 报告，记录每条挑战样本来自哪个原始样本、采用了什么扰动方式。

```powershell
wikieval mutate `
  --dataset benchmarks/smoke/cases.jsonl `
  --output artifacts/challenges/round6-challenge.jsonl `
  --report artifacts/challenges/round6-report.json
```

当前 mutation 主要做三类事情：

- 在技术问答和 Wiki 任务里加入跨域干扰，测试路由是否会被误导；
- 在电商风险问题里加入技术链路干扰，测试业务决策是否仍能聚焦关键指标；
- 保留原样本的 gold 语义，只把输入改得更像真实线上追问。
